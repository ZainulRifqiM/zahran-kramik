...
$bayar = query("SELECT * FROM pembayaran,transaksi WHERE pembayaran.NoFaktur=transaksi.NoFaktur AND pembayaran.IdPembayaran = $IdPembayaran")[0];
...
<div class="container" style="">
	...
	<form action="" method="post" enctype="multipart/form-data">
		...
		<div class="mb-3">
			...
			<Select name="NoFaktur" class="form-control form-select" value="<?= $bayar["NoFaktur"] ?>" style="width: 400px">
				<?php
				$dataFaktur = getDataTransaksi();
				foreach ($dataFaktur as $row) :
				?>
					<option id="NoFaktur" value="<?= $row["NoFaktur"] ?>" <?php if ($bayar["NoFaktur"] == $row["NoFaktur"]) {
																				echo 'selected';
																			}  ?>><?= $row["NoFaktur"] ?>
