	...
	<form action="" method="post" enctype="multipart/form-data">
		...
		<div class="mb-3">
			...
			<Select name="IdSales" class="form-control form-select" value="<?= $transaksi["NamaSales"] ?>" style="width: 400px">
				...
				?>
					<option id="IdSales" value="<?= $row["IdSales"] ?>" <?php if ($transaksi["IdSales"] == $row["IdSales"]) {
																			echo 'selected';
																		}  ?>><?= $row["NamaSales"] ?></option>
				<?php endforeach; ?>
			</Select>
		</div>
		...
		<div class="mb-3">
			...
			<Select name="IdPembeli" class="form-control form-select" value="<?= $transaksi["NamaPembeli"] ?>" style="width: 400px">
				...
				?>
					<option id="IdPembeli" value="<?= $row["IdPembeli"] ?>" <?php if ($transaksi["IdPembeli"] == $row["IdPembeli"]) {
																				echo 'selected';
																			}  ?>><?= $row["NamaPembeli"] ?></option>
				<?php endforeach; ?>
			</Select>
		</div>
		...
		<div class="mb-3">
			...
			<Select name="IdBarang" class="form-control form-select" value="<?= $transaksi["NamaBarang"] ?>" style="width: 400px">
				...
				?>
					<option id="IdBarang" value="<?= $row["IdBarang"] ?>" <?php if ($transaksi["IdBarang"] == $row["IdBarang"]) {
																				echo 'selected';
																			}  ?>><?= $row["NamaBarang"] ?></option>
				<?php endforeach; ?>
			</Select>
		</div>
